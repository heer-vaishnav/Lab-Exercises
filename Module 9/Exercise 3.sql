...
BEGIN
	...
	WHERE p.Color IS NOT NULL ORDER BY p.Color;
END
GO
...
BEGIN
	...
	ON pm.ProductModelID = ed.ProductModelID AND ed.LanguageID = 'en'
...
BEGIN
	...
	FROM Marketing.Product AS p WHERE (p.Color = @Color) OR (p.Color IS NULL AND @Color IS NULL)
